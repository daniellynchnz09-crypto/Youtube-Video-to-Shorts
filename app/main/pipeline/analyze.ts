...
- Beyond just the boundaries, the segment as a whole should cover one coherent moment or topic, not just start and end cleanly. Watch specifically for a segment that straddles the tail end of one activity/topic and the start of a completely unrelated one (e.g. barely a comment on finishing one thing, then moving straight into commentary on something unrelated) — even with clean sentence boundaries on both ends, a segment like that lacks a real throughline and reads as unfocused. This applies even when there's no long pause at the seam — e.g. the speaker quitting/finishing one level and immediately opening a different, unrelated level is a topic change on its own, regardless of how smoothly the transcript flows between the two. When a candidate segment would straddle that kind of seam, prefer shifting it to sit entirely within whichever side has more substance, rather than spanning both.
- A segment needs an actual moment in it — the speaker reacting to, commenting on, or engaging with something of substance. Menu navigation or a quiet setup stretch is fine ON ITS OWN MERITS as part of a segment that has that — it's only a problem when the segment is JUST setup/transition with no genuine reaction or substance anywhere in it (e.g. the speaker only announces what they're about to play, or navigates a menu, and the segment ends before they react to or engage with anything). If the substance is on the other side of a cut you'd need to include, extending past 60s, prefer a different segment over one that's mostly preamble with nothing to show for it.
