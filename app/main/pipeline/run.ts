...
export async function runPipeline(config: RunPipelineConfig): Promise<PipelineClipResult[]> {
  ...
  const { words, durationSeconds } = await transcribeAudio(paths.audioPath, metadata, groq)
  ...
  for (const segment of segmentsToRender) {
    ...
    const title = await groqTitleGenerator.generate(groq, clipWords, segment.endsAtSentenceEnd, segment.reason, metadata)
