...
- **Partly done (2026-09-02):** Whisper's `initial_prompt` (via WhisperX's `asr_options`) is now fed the source video's title + description on every transcription — see [`buildTranscriptionHint`](../app/main/pipeline/videoContext.ts) and [spec.md](spec.md#title-generation-context). This biases spelling of proper nouns the model would otherwise mangle (content and creator names). It's a soft bias with a ~224-token budget, so it's not a full fix.
...
Idea proposed 2026-08-20, sits alongside the [Custom dictionary](spec.md#editing-in-review) feature but is broader: the channel covers one specific video game with its own in-game vocabulary (e.g. an obstacle the game calls one thing that Whisper/the LLM defaults to a more generic word for), so a per-game context profile could correct terminology at both the transcription-correction and title-generation stages.

**Partial groundwork already in place (2026-09-02):** the source video's metadata — including its tags — is now fetched and fed into the analyzer/title prompts and the transcription `initial_prompt` hint (see [spec.md](spec.md#title-generation-context)). The uploader's tags are effectively a per-video term list. A hand-maintained game glossary data file (local, gitignored; see `glossary.example.json`) is seeded as the deliberately-simple precursor — starter terminology mappings live there now.
...
- **Jargon / mechanics / slang** — *user-defined* (optionally LLM-drafted, user-approved). Community vernacular the wiki doesn't document; it changes slowly; and it's the terms that most affect a title's *tone*. This tier is what the hand-seeded glossary covers today.
- **Content & people** — *wiki-sourced*, auto-fetched and refreshable. The game's community wiki (Fandom, MediaWiki API) is written by people deep in the community and is kept current — difficulty-ranking placements move, people climb rankings, content gets re-verified. A static user-written entry goes stale; a wiki-sourced entry can be re-pulled. Store each with its source URL + fetch date so it can be refreshed. **Deliberately scoped to content and people only** — not jargon, where the disambiguation problem is worst (many proper nouns are also ordinary words) and the wiki has least to say.

**Unknown-term detection** (what populates the "needs a definition" list):
- proper-noun-ish tokens — capitalized mid-sentence, or appearing in the video's tags / **chapter titles** (creator timestamps; yt-dlp *does* expose these and they carry correct content-name spellings — worth wiring into the metadata hint even before the full feature) — that aren't already in the glossary;
- plus a per-transcript LLM pass: "list the game-specific jargon and proper nouns here that aren't in this glossary" (this is what `findUnknownTermCandidates` in `glossary.ts` approximates with heuristics today).
- The GUI presents these grouped by category (people / content / mechanics / …) and the user resolves each: accept a wiki/LLM draft, edit it, add spelling variants, or dismiss.

**Wiki lookup mechanics:** search the term → pick the right page (LLM-assisted disambiguation, using the transcript sentence as context) → LLM compresses the article prose into a short usable definition → present for user approval. Wiki content also grounds the LLM so it doesn't hallucinate specifics (who did what, exact ranking placement).
...
**Auto-triggered on every candidate term (2026-09-18).** `flag-terms.ts` (the new-video workflow's flagging step, see [spec.md](spec.md#new-video-workflow)) now runs `lookupTermOnWiki()` automatically for every `findUnknownTermCandidates` result instead of just listing them — no more manual per-term CLI calls needed for the common case. Context for disambiguation comes from `findTermContext()` (new in `glossary.ts`): finds the candidate term's first occurrence in the word-timestamp array (not a plain string search, so a multi-word term only matches consecutive spoken words) and returns a padded window around it. Verified against a real video: correctly auto-drafted two terms with cited sources (one of them a gamemode name that turned out to be a real glossary gap, not just a one-off) and correctly routed seven others — generic words, and level names genuinely absent from the wiki — to "needs your input" rather than forcing a bad match.

Still not implemented: the GUI (still gated on build-order step 4+) and asset recognition.

## Batch segment selection clusters on the same handful of moments

Observed during build-order step 1 review testing (2026-08-28), across roughly 10 separate `analyze()` calls on the same ~16-minute video: a small "core" set of ~5 moments (an intro bit, two mid-video commentary bits, a rant, and the closing teaser) came back as a candidate in nearly *every* run, while a genuine "long tail" of other well-reasoned candidates (five or so distinct moments elsewhere in the video) only surfaced in *some* runs — seemingly displacing one of the "core" five rather than the model exploring differently each time.
