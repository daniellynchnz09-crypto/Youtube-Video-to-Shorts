...
Electron App (runs only while the user is using it)
  ...
  │      afterward, not fetched piecemeal — see the note below); also a
  │      metadata-only fetch (title/description/tags, no media) that feeds
  │      the analyzer + title prompts as context
  ...
  app/
    main/                  # Electron main process
      pipeline/            # download, transcribe, analyze, render steps
                            ...
                            #   providers" above
                            #   glossary.example.json — template for the
                            #   channel's game-term data; the real
                            #   glossary.json is local + gitignored
                            #   (glossary.ts loads/matches it)
